import { __toESM, require_react } from "./chunk-BJFMU22U.js";

// node_modules/.deno/react-router-dom@6.4.3/node_modules/react-router-dom/dist/index.js
...
function convertRoutesToDataRoutes(routes, parentPath, allIds) {
  ...
  return routes.map((route, index) => {
    ...
    invariant(
      !allIds.has(id),
      'Found a route id collision on id "' + id +
...
function rankRouteBranches(branches) {
  branches.sort((a, b) =>
    a.score !== b.score ? b.score - a.score : compareIndexes(
...
function compareIndexes(a, b) {
  let siblings = a.length === b.length &&
...
function matchPath(pattern, pathname) {
  ...
  let params = paramNames.reduce((memo, paramName, index) => {
    if (paramName === "*") {
      ...
      pathnameBase = matchedPathname.slice(
        0,
...
function safelyDecodeURIComponent(value, paramName) {
  try {
    ...
    warning(
      false,
      'The value for the URL param "' + paramName +
...
function getInvalidPathError(char, field, dest, path) {
  return "Cannot include a '" + char + "' character in a manually specified " +
    ("`to." + field + "` field [" + JSON.stringify(path) +
...
function resolveTo(toArg, routePathnames, locationPathname, isPathRelative) {
  ...
  let hasExplicitTrailingSlash = toPathname && toPathname !== "/" &&
    toPathname.endsWith("/");
  let hasCurrentTrailingSlash = (isEmptyPath || toPathname === ".") &&
...
var DeferredData = class {
  ...
  onSettle(promise, key, error, data) {
    if (
      this.controller.signal.aborted && error instanceof AbortedDeferredError
    ) {
      this.unlistenAbortSignal();
...
function createRouter(init) {
  ...
  let initialized = !initialMatches.some((m) => m.route.loader) ||
  ...
  function completeNavigation(location, newState) {
    ...
    let isActionReload = state.actionData != null &&
      ...
      ((_state$navigation$for = state.navigation.formAction) == null
          ? void 0
  ...
  async function navigate(to, opts) {
    ...
    let historyAction = (opts && opts.replace) === true || submission != null
      ? Action.Replace
      : Action.Push;
  ...
  async function handleFetcherLoader(key, routeId, path, match, matches) {
    ...
    if (isDeferredResult(result)) {
      result = await resolveDeferredData(result, fetchRequest.signal, true) ||
        result;
    }
    if (fetchControllers.get(key) === abortController) {
      fetchControllers.delete(key);
...
var validActionMethods = /* @__PURE__ */ new Set([
  "POST",
  "PUT",
  "PATCH",
  "DELETE",
]);
var validRequestMethods = /* @__PURE__ */ new Set([
  "GET",
  "HEAD",
  ...validActionMethods,
]);
function normalizeNavigateOptions(to, opts, isFetcher) {
  ...
  if (opts.formMethod != null && opts.formMethod !== "get") {
    return {
      ...
      submission: {
        ...
        formEncType: opts && opts.formEncType ||
  ...
  try {
    ...
    if (
      isFetcher && parsedPath.search && hasNakedIndexQuery(parsedPath.search)
    ) {
...
function isNewRouteInstance(currentMatch, match) {
  ...
  return currentMatch.pathname !== match.pathname ||
...
function shouldRevalidateLoader(
  ...
  let defaultShouldRevalidate = isNewRouteInstance(currentMatch, match) ||
    ...
    currentUrl.search !== nextUrl.search || isRevalidationRequired;
...
async function callLoaderOrAction(
  ...
  try {
    ...
    invariant(
      handler,
      "Could not find the " + type + ' to run on the "' + match.route.id +
        '" route',
    );
  ...
  if (result instanceof Response) {
    ...
    if (status >= 300 && status <= 399) {
      ...
      if (basename) {
        ...
        resolvedLocation.pathname = path === "/"
          ? basename
          : joinPaths([basename, path]);
      }
...
function createRequest(location, signal, submission) {
  ...
  if (submission) {
    ...
    init.body = formEncType === "application/x-www-form-urlencoded"
...
function processRouteLoaderData(
  ...
  results.forEach((result, index) => {
    ...
    if (isErrorResult(result)) {
      ...
      if (
        result.statusCode != null && result.statusCode !== 200 && !foundError
      ) {
...
function findNearestBoundary(matches, routeId) {
  let eligibleMatches = routeId
    ...
    : [...matches];
  return eligibleMatches.reverse().find((m) =>
    m.route.hasErrorBoundary === true
  ) || matches[0];
}
...
async function resolveDeferredResults(
  ...
  for (let index = 0; index < results.length; index++) {
    ...
    let currentMatch = currentMatches.find((m) =>
      m.route.id === match.route.id
    );
    let isRevalidatingLoader = currentMatch != null &&
...
function getTargetMatch(matches, location) {
  let search = typeof location === "string"
    ? parsePath(location).search
    : location.search;
  if (
    matches[matches.length - 1].route.index && hasNakedIndexQuery(search || "")
  ) {
...
// node_modules/.deno/react-router@6.4.3/node_modules/react-router/dist/index.js
...
var useSyncExternalStore = "useSyncExternalStore" in React
  ...
  if (true) {
    ...
    warningOnce(
      ...
      "You rendered descendant <Routes> (or called `useRoutes()`) at " +
        ('"' + parentPathname + '" (under <Route path="' + parentPath +
          '">) but the ') +
...
` + ('Please change the parent <Route path="' + parentPath + '"> to <Route ') +
  ...
  let remainingPathname = parentPathnameBase === "/"
    ? pathname
  ...
  if (true) {
    true
      ? warning(
        ...
        'No routes matched location "' + location.pathname + location.search +
          location.hash + '" ',
      )
      : void 0;
    true
      ? warning(
        ...
        'Matched leaf route at location "' + location.pathname +
...
function DefaultErrorElement() {
  ...
  return React.createElement(
    React.Fragment,
    null,
    React.createElement("h2", null, "Unhandled Thrown Error!"),
    ...
    stack
      ...
      : null,
    React.createElement("p", null, "\u{1F4BF} Hey developer \u{1F44B}"),
    React.createElement(
      "p",
      null,
...
function _renderMatches(matches, parentMatches, dataRouterState) {
  ...
  if (errors != null) {
    let errorIndex = renderedMatches.findIndex((m) =>
      m.route.id && (errors == null ? void 0 : errors[m.route.id])
    );
    !(errorIndex >= 0)
      ? true
        ? invariant(
  ...
  return renderedMatches.reduceRight((outlet, match, index) => {
    let error = match.route.id
      ? errors == null ? void 0 : errors[match.route.id]
      : null;
    let errorElement = dataRouterState
      ? match.route.errorElement ||
        React.createElement(DefaultErrorElement, null)
      : null;
    let getChildren = () =>
      React.createElement(
        RenderedRoute,
...
function useLoaderData() {
  ...
  !route
    ? true
      ? invariant(false, "useLoaderData must be used inside a RouteContext")
      : invariant(false)
    : void 0;
...
function Routes(_ref5) {
  ...
  let routes = dataRouterContext && !children
...
function shouldProcessLinkClick(event, target) {
  return event.button === 0 && (!target || target === "_self") &&
...
function getFormSubmissionInfo(target, defaultAction, options) {
  ...
  if (isFormElement(target)) {
    ...
    encType = options.encType || target.getAttribute("enctype") ||
      defaultEncType;
    ...
    method = options.method || target.getAttribute("formmethod") ||
      form.getAttribute("method") || defaultMethod;
    action = options.action || target.getAttribute("formaction") ||
      form.getAttribute("action") || defaultAction;
    encType = options.encType || target.getAttribute("formenctype") ||
      form.getAttribute("enctype") || defaultEncType;
  ...
  return createRouter({
    ...
    hydrationData: (opts == null ? void 0 : opts.hydrationData) ||
      ((_window2 = window) == null
        ? void 0
        : _window2.__staticRouterHydrationData),
    routes: enhanceManualRouteObjects(routes),
  }).initialize();
}
...
function useLinkClickHandler(to, _temp) {
  ...
  return React2.useCallback((event) => {
    if (shouldProcessLinkClick(event, target)) {
      event.preventDefault();
      let replace = replaceProp !== void 0
        ? replaceProp
...
function useScrollRestoration(_temp3) {
  ...
  React2.useLayoutEffect(() => {
    let disableScrollRestoration = router == null
      ? void 0
      : router.enableScrollRestoration(savedScrollPositions, () =>
        window.scrollY, getKey);
